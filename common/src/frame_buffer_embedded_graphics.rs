...
use crate::screen_info::ScreenInfoWithAddress;

pub struct FrameBufferEmbeddedGraphics<'a> {
    screen: &'a mut ScreenInfoWithAddress,
}
...
impl<'a> FrameBufferEmbeddedGraphics<'a> {
    pub fn new(screen: &'a mut ScreenInfoWithAddress) -> Result<Self, NewError> {
    ...
    fn frame_buffer_mut(&mut self) -> &mut [u8] {
        // Safety: The memory is  to the frame buffer and nothing else is referencing it
        unsafe {
    ...
    pub fn shift_up(&mut self, amount: u32) {
        let pitch = self.screen.info.pitch;
...
impl DrawTarget for FrameBufferEmbeddedGraphics<'_> {
    ...
    fn clear(&mut self, color: Self::Color) -> Result<(), Self::Error> {
        ...
        for pixel_index in 0..(self.screen.info.width * self.screen.info.height) as usize {
            self.frame_buffer_mut()[pixel_index * 4..pixel_index * 4 + 4].copy_from_slice(&pixel);
        }
        Ok(())
    }
}
