...
pub const PHYS_MEM_TRACKER: OnceCell<Spinlock<ContinuousBoolVec<Vec<usize>>>> = OnceCell::uninit();
pub const KERNEL_ADDRESS_SPACE_TRACKER: OnceCell<Spinlock<ContinuousBoolVec<Vec<usize>>>> =
    OnceCell::uninit();
    ...
    pub fn new(memory_map_response: &'static MemoryMapResponse, hhdm_offset: u64) -> Self {
        ...
        let up_to_n_virt_entries_after_allocating = {
            let up_to_n_virt_entries =
                1 + unsafe { PageTableDeepIterator::new(hhdm_offset) }.count();
        ...
        PHYS_MEM_TRACKER
            .try_init_once(|| {
                Spinlock::new(ContinuousBoolVec {
                    ...
                    len_vec: {
                        let (ptr, len, cap) = phys_mem_tracker.len_vec.into_raw_parts();
        ...
        KERNEL_ADDRESS_SPACE_TRACKER
            .try_init_once(|| {
                Spinlock::new(ContinuousBoolVec {
                    ...
                    len_vec: {
                        let (ptr, len, cap) = kernel_address_space_tracker.len_vec.into_raw_parts();
                        unsafe { Vec::from_raw_parts(ptr, len, cap) }
                    },
                })
            })
...
pub fn init(memory_map_response: &'static MemoryMapResponse, hhdm_offset: u64) {
    ALLOCATOR
        .try_init_once(|| PtAllocator::new(memory_map_response, hhdm_offset))
        .unwrap();
}
