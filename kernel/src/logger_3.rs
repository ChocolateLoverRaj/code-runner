    ...
    fn log(&self, record: &log::Record) {
        ...
        if let Some(serial_port) = &mut data.serial_port {
            ...
            if record.level() <= log_serial_config.level_filter {
                ...
                let level: &dyn Display = if log_serial_config.colors {
                ...
                writeln!(writer, "[{:?}] {:5}: {}", CpuIdDebug, level, record.args()).unwrap();
            }
        }
        if let Some(frame_buffer_data) = &mut data.frame_buffer {
            ...
            if record.level() <= log_screen_config.level_filter {
                let font = log_screen_config.font;
                ...
                write!(writer, "[{:?}] ", CpuIdDebug);
                writer.text_color = match record.level() {
